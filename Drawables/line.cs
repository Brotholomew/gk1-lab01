...
namespace lab01
{
    public class line : drawable
    {
        ...
        public override void Move(MouseEventArgs e, Point distance, Relation sanitizer, MovingOpts mo)
        {
            ...
            if (mo.Solo)
            {
                // if line is being moved directly, move vertices as well
                mo.Solo = false;
        ...
        public void MakeParallel(line l, double oldLength)
        {
            ...
            {
                // calculate new coordinates for both vertices
                double a = pattern.a;
                ...
                else if (double.IsInfinity(pattern.a))
                {
                    nXE = oldStart.X;
                    nXS = oldEnd.X;
                    nYE = oldStart.Y - (int)this.Length;
                    nYS = oldEnd.Y - (int)this.Length;
                }
                else
                {
                ...
                // move this vertex that will be placed closer to its original location
